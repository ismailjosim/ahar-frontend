    ...
    <Dialog open={open} onOpenChange={onClose}>
      <DialogContent className="sm:max-w-3xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>View complete information about this menu item.</DialogDescription>
        </DialogHeader>
        ...
        <div className="space-y-6">
          ...
          <div className="grid gap-4 md:grid-cols-2">
            ...
            <div className="space-y-1">
              ...
              <p className="font-medium text-lg text-primary">{formatCurrency(menuItem.price)}</p>
            </div>

            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Rating</p>
              <p className="font-medium">
                {menuItem.rating ? <span>⭐ {menuItem.rating.toFixed(1)} / 5</span> : "No rating"}
              </p>
            </div>
            ...
            {menuItem.prepTime && (
              <div className="space-y-1">
                ...
                <p className="font-medium">{menuItem.prepTime}</p>
              </div>
            )}
          </div>
          ...
          {menuItem.description && (
            <div className="space-y-2">
              ...
              <p className="whitespace-pre-wrap leading-relaxed">{menuItem.description}</p>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex flex-wrap gap-2">
              {menuItem.isFeatured && <Badge variant="default">Featured</Badge>}
              {menuItem.isSpicy && <Badge variant="secondary">🌶️ Spicy</Badge>}
              ...
              <div className="space-y-2">
                {menuItem.variants.map((variant, index) => (
                  ...
                  >
                    ...
                    <span className="text-sm font-semibold text-primary">{/* +{formatCurrency(variant)} */}</span>
                  </div>
                ))}
              </div>
            </div>
          ...
          <div className="grid gap-4 md:grid-cols-2 pt-4 border-t">
            <div className="space-y-1">
              ...
              <p className="font-medium text-sm">{new Date(menuItem.createdAt).toLocaleString()}</p>
            </div>

            <div className="space-y-1">
              <p className="text-sm text-muted-foreground">Updated At</p>
              <p className="font-medium text-sm">{new Date(menuItem.updatedAt).toLocaleString()}</p>
            </div>
          </div>
        </div>
      </DialogContent>
